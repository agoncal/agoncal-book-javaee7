 ...
 *         --
 */
public class CardValidator06IT {

  @Test
  public void shouldCheckCreditCardValidity() throws MalformedURLException {

    Endpoint endpoint = Endpoint.publish("http://localhost:8080/cardValidator", new CardValidator06());
    assertTrue(endpoint.isPublished());
    assertEquals("http://schemas.xmlsoap.org/wsdl/soap/http", endpoint.getBinding().getBindingID());

    String url = "http://localhost:8080/cardValidator?wsdl";
    String namespace = "http://ex06.chapter21.javaee7.book.agoncal.org/";
    QName serviceQN = new QName(namespace, "CardValidator06Service");
    Service service = Service.create(new URL(url), serviceQN);
    String portName = "CardValidator06Port";
    QName portQN = new QName(namespace, portName);

    Validator06 cardValidator = service.getPort(portQN, Validator06.class);
