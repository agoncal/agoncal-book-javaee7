...
//    assertEquals("ping", client.target("http://localhost:8282/15/customer/ping").request(MediaType.TEXT_PLAIN).get(String.class));
    assertEquals("ping", client.target("http://localhost:8080/chapter22-service-1.0/rs/15/customer/ping").request(MediaType.TEXT_PLAIN).get(String.class));
  }

  @Test
  public void getGetDefaultMediaType() {
//    Response response = client.target("http://localhost:8282/15/customer/locale").request().get();
    Response response = client.target("http://localhost:8080/chapter22-service-1.0/rs/15/customer/media").request().get();
...
//    Response response = client.target("http://localhost:8282/15/customer/locale").request().get();
    Response response = client.target("http://localhost:8080/chapter22-service-1.0/rs/15/customer").request().post(Entity.entity(new Customer15(1L, "John", "Smith", "jsmith@gmail.com", "1234565"), MediaType.APPLICATION_XML));
    assertEquals(201, response.getStatus());
    assertEquals("http://localhost:8080/chapter22-service-1.0/rs/15/customer/1",response.getLocation().toString());
  }
}